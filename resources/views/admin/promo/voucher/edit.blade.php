...
@section('container')
    ...
    <div class="alert-notification-wrapper" id="alert-notification-wrapper">
        ...
        <form action="{{ route('promovoucher.update', $promo) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="card promoVoucher-info-card border-0 border-radius-1-5rem fs-14">
                    ...
                    <div class="card-body p-4 pt-0">
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-9">
                                <input required type="text"
                                    class="form-control fs-14 @error('name') is-invalid @enderror" id="promoVoucherName"
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-9">
                                <input required type="text"
                                    class="form-control fs-14  @error('code') is-invalid @enderror" id="promoVoucherCode"
                                    name="code" id="promo_voucher_code" maxlength="12" placeholder="Contoh: SPL63"
                                    value="{{ !is_null($promo->code) ? (!is_null(old('code')) ? old('code') : $promo->code) : (!is_null(old('code')) ? old('code') : $promo->code) }}">
                                @error('code')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                        ...
                        <div class="mb-3 row align-items-center">
                            <label for="promoVoucherStartPeriod" class="col-sm-3 col-form-label fw-600">Periode
                                Promo</label>
                            <div class="col-sm-4">
                                <input required type="datetime-local"
                                    ...
                                    value="{{ !is_null($promo->start_period) ? (!is_null(old('start_period')) ? old('start_period') : $promo->start_period) : (!is_null(old('start_period')) ? old('start_period') : $promo->start_period) }}">
                                @error('start_period')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                            ...
                            <div class="col-sm-4">
                                <input required type="datetime-local"
                                    ...
                                    value="{{ !is_null($promo->end_period) ? (!is_null(old('end_period')) ? old('end_period') : $promo->end_period) : (!is_null(old('end_period')) ? old('end_period') : $promo->end_period) }}"
                ...
                <div class="card promoVoucher-info-card border-0 border-radius-1-5rem fs-14">
                    <div class="card-header bg-transparent p-4 border-0">
                        <div class="header">
                            ...
                            <p class="text-grey fs-13 m-0">Isikan Jenis/Tipe, diskon, min.transaksi, kuota, dan deskripsi/
                                S&K
                                promo</p>
                        </div>
                    </div>
                    <div class="card-body p-4 pt-0">
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-9">
                                <input required type="number"
                                    ...
                                    value="{{ !is_null($promo->discount) ? (!is_null(old('discount')) ? old('discount') : $promo->discount) : (!is_null(old('discount')) ? old('discount') : $promo->discount) }}">
                                @error('discount')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-9">
                                <input required type="number"
                                    ...
                                    value="{{ !is_null($promo->min_transaction) ? (!is_null(old('min_transaction')) ? old('min_transaction') : $promo->min_transaction) : (!is_null(old('min_transaction')) ? old('min_transaction') : $promo->min_transaction) }}">
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-9">
                                <input required type="number"
                                    class="form-control fs-14 @error('quota') is-invalid @enderror" id="promoVoucherQuota"
                                    name="quota" placeholder="Masukkan batas pemakaian promo voucher, contoh: 5"
                                    value="{{ !is_null($promo->quota) ? (!is_null(old('quota')) ? old('quota') : $promo->quota) : (!is_null(old('quota')) ? old('quota') : $promo->quota) }}">
                                @error('quota')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-9">
                                <input id="promoVoucherDescription" type="hidden" name="description"
                                    value="{{ !is_null($promo->description) ? (!is_null(old('description')) ? old('description') : $promo->description) : (!is_null(old('description')) ? old('description') : $promo->description) }}">
            ...
            <div class="container promo-payment-method-container p-0 mb-4">
                <div class="card promo-payment-method-card border-0 border-radius-1-5rem fs-14">
                    <div class="card-header bg-transparent p-4 border-0">
                        <div class="header">
                        ...
                        @foreach ($paymentMethods as $paymentMethod)
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" value="{{ $paymentMethod->id }}"
                                    ...
                                    @foreach ($promo->promopaymentmethod as $paymentmethod){{ $paymentmethod->payment_method_id == $paymentMethod->id ? 'checked' : '' }} @endforeach>
                                    ...
                                    <div class="row align-items-center">
                                        @if (!empty($paymentMethod->logo))
                                            <div class="col-md-1 col-2 pe-0">
                                                <img class="w-100" src=" {{ asset($paymentMethod->logo) }}"
                                                    alt="">
                                            </div>
            ...
            <div class="container promo-status-container p-0 mb-4">
                <div class="card promo-status-card border-0 border-radius-1-5rem fs-14">
                    <div class="card-header bg-transparent p-4 border-0">
                        <div class="header">
                            ...
                            <div class="col-sm-9 py-1">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" role="switch"
                                        id="promoVoucherStatus" name="is_active" value="1"
                                    ...
                                    <select id="promoVoucherProducts"
                                        ...
                                        @foreach ($products as $product)
                                            <option value="{{ $product->id }}"
                                                @foreach ($promo->productpromo as $productPromo) {{ $productPromo->product_id == $product->id ? 'selected' : '' }} @endforeach>
                                                {{ $product->name }}</option>
                                        @endforeach
                                    </select>
            ...
            <div class="modal fade" id="upload-promoVoucher-img-modal" data-bs-backdrop="static"
                data-bs-keyboard="false" tabindex="-1" aria-labelledby="previewImgUserModal" aria-hidden="true">
        ...
        <script>
            ...
            $(document).ready(function() {
                console.log($('#promoVoucherPromoType').find(':selected'));
                ...
                $('.promo-type').val(parseInt(promo['promo_type_id'])).trigger('change');
                ...
                $('body').on('click', '.btn-delete-image', function(e) {
                    ...
                    var promoVoucherId = e.currentTarget.id;
                    var token = $('meta[name="csrf-token"]').attr('content');
                    console.log(token);
                    deleteImage(token, promoVoucherId);
                });

                function deleteImage(csrfToken, promoVoucherId) {
                    $.ajax({
                        url: "{{ url('/administrator/promovoucher/deleteimage') }}",
                        type: 'post',
                        data: {
                            _token: csrfToken,
                            id: promoVoucherId,
                ...
                $("body").on("click", ".preview-promo-Voucher-image-button", function(e) {
                    ...
                    // var clone = $(this).clone();
                    // clone.attr("name","promoVoucherImageTemp["+0+"]").addClass('d-none');
                    // $('#imageSource_'+imageId).html(clone);
